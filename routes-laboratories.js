...
router.post('/laboratory_add', requireLoggedIn, upload.array('attachments[]'), function (req, res) {
	...
	if (req.body.notes.trim() === "")
		req.body.notes = null;
	// console.log(req.session.lab_results_attachment);
	Laboratory.create({
		date: req.body.date,
...
router.post('/upload_files_lab_results', function (req, res) {
	upload_success (req, res, function (err) {
		...
		// req.session.lab_results_attachment.push(req.files[0].path);
		console.log(fileQueue[fileId].filesArr);
		res.json({});
	});
});
